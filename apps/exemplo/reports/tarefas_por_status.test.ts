...
describe("Relatório Tarefas por Status", () => {
  it("uma linha por status, com percentual e gráfico", () => {
    ...
    const r = relatorio.execute({ projeto: p.name }, cerne.session);
  ...
  it("filtra por responsável e por data limite", () => {
    ...
    const ate = relatorio.execute({ projeto: p.name, data_limite_ate: u().addDays(u().today(), 10) }, cerne.session);
    expect(ate.totals!.quantidade).toBe(1);

    const outro = relatorio.execute({ projeto: p.name, responsavel: "Guest" }, cerne.session);
