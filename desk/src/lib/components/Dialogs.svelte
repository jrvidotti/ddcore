<script lang="ts">
  ...
  function layout(fields: any[]) {
    ...
    for (const f of fields) {
      ...
      else sections[sections.length - 1].push(f);
    }
    return sections;
  }

  /** A modal is too narrow for quarter-line cells, so its lines are half as wide. */
  ...
  <div class="modal-bg" role="dialog" aria-modal="true" onkeydown={(e) => e.key === "Escape" && cancel(d)} tabindex="-1">
    <div class="modal {d.spec.size || 'md'}">
      ...
      <div class="body">
        ...
        {#each layout(d.spec.fields || []) as fields}
          {#each packLines(fields, undefined, DIALOG_SLOTS) as line}
            <div class="form-row">
