...
describe("Single forms", () => {
 it("loads defaults from the server even on the new route", async () => {
  ...
  vi.mocked(api.getSingle).mockResolvedValue({doctype:"Settings",id:"singleton",__islocal:true,enabled:true});
 ...
 it("keeps a reader's unsaved settings read-only even with create", () => {
  const frm = new FormController(meta(false), {id:"singleton",__islocal:true});
 ...
 it("is not new before its first save", () => {
  const frm = new FormController(meta(true), {doctype:"Settings",id:"singleton",__islocal:true,enabled:true});
 ...
 it("uses PUT for the first save", async () => {
  const frm = new FormController(meta(true), {doctype:"Settings",id:"singleton",__islocal:true,enabled:false});
  vi.mocked(api.update).mockResolvedValue({doctype:"Settings",id:"singleton",enabled:false});
