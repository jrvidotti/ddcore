...
func castValue(f *meta.Field, v any) (any, error) {
	...
	case "Date":
		...
		if _, err := time.Parse("2006-01-02", s); err != nil {
			return nil, cerr.Validation("Data inválida em {0}: \"{1}\"", f.Label, db.Str(v))
		}
		return s, nil
	case "Month":
		s := db.Str(v)
		...
		return nil, cerr.Validation("Mês/ano inválido em {0}: \"{1}\"", f.Label, db.Str(v))
	case "Datetime":
		...
		return nil, cerr.Validation("Data/hora inválida em {0}: \"{1}\"", f.Label, s)
	case "Time":
		return db.Str(v), nil
	case "JSON":
...
func (c *Ctx) getDoc(doctype, name string, forUpdate bool) (Doc, error) {
	...
	if name == "" {
		return nil, cerr.NotFound("{0}: nome vazio", doctype)
	}
	...
	if len(rows) == 0 {
		return nil, cerr.NotFound("{0} {1} não encontrado", c.T(d.Label), name)
	}
	doc := Doc(rows[0])
	doc["doctype"] = doctype
	...
	if !c.IgnorePermissions() {
		if ok, err := c.HasPermission(doctype, "read", doc); err != nil {
			...
			return nil, cerr.Permission("Sem permissão para ler {0} {1}", c.T(d.Label), name)
		}
	}
	return doc, nil
}
...
func (c *Ctx) Insert(doc Doc, opts SaveOpts) (Doc, error) {
	...
	if d.IsChild {
		return nil, cerr.Validation("{0} é uma tabela filha", d.Name)
	}
	if !opts.IgnorePermissions && !c.IgnorePermissions() {
		if ok, err := c.HasPermission(d.Name, "create", doc); err != nil {
			return nil, err
		} else if !ok {
			return nil, cerr.Permission("Sem permissão para criar {0}", c.T(d.Label))
		}
	}
	if doc.Docstatus() == 1 && d.Submittable {
		if !opts.IgnorePermissions && !c.IgnorePermissions() {
			if ok, _ := c.HasPermission(d.Name, "submit", doc); !ok {
				return nil, cerr.Permission("Sem permissão para enviar {0}", c.T(d.Label))
			}
		}
...
func (c *Ctx) Save(doc Doc, opts SaveOpts) (Doc, error) {
	...
	case oldStatus == 2:
		return nil, cerr.Validation("{0} {1} está cancelado e não pode ser alterado", c.T(d.Label), doc.Name())
	case oldStatus == 0 && newStatus == 0:
	default:
		return nil, cerr.Validation("Transição de docstatus inválida ({0} → {1})", oldStatus, newStatus)
	}
	if action != "save" && !d.Submittable {
		return nil, cerr.Validation("{0} não é submetível", c.T(d.Label))
	}
	if !opts.IgnorePermissions && !c.IgnorePermissions() {
		ptype := "write"
		if action == "submit" {
		...
		if ok, err := c.HasPermission(d.Name, ptype, before); err != nil {
			...
			return nil, cerr.Permission("Sem permissão ({0}) em {1} {2}", ptype, c.T(d.Label), doc.Name())
		}
	}
...
func (c *Ctx) Amend(doctype, name string) (Doc, error) {
	...
	if ok, _ := c.HasPermission(doctype, "amend", src); !ok && !c.IgnorePermissions() {
		return nil, cerr.Permission("Sem permissão para emendar {0}", c.T(d.Label))
	}
	doc := src.Clone()
...
func (c *Ctx) DBSet(doctype, name string, values Doc, updateModified bool) (time.Time, error) {
	...
	for k, v := range values {
		...
		if f == nil || meta.ColumnType(f.Fieldtype) == "" {
			if !d.IsStdColumn(k) {
				return modified, cerr.Validation("Campo {0} não existe em {1}", k, doctype)
			}
	...
	if tag.RowsAffected() == 0 {
		return modified, cerr.NotFound("{0} {1} não encontrado", doctype, name)
	}
...
func (c *Ctx) Delete(doctype, name string, ignorePerms, force bool) error {
	...
	if !ignorePerms && !c.IgnorePermissions() {
		if ok, _ := c.HasPermission(doctype, "delete", doc); !ok {
			return cerr.Permission("Sem permissão para apagar {0} {1}", c.T(d.Label), name)
		}
	}
	if doc.Docstatus() == 1 {
		return cerr.Validation("Cancele {0} {1} antes de apagar", c.T(d.Label), name)
	}
...
func (c *Ctx) Rename(doctype, oldName, newName string) (string, error) {
	...
	if !d.AllowRename {
		return "", cerr.Validation("{0} não permite renomear", c.T(d.Label))
	}
	...
	if ok, _ := c.HasPermission(doctype, "write", doc); !ok && !c.IgnorePermissions() {
		return "", cerr.Permission("Sem permissão para renomear {0}", c.T(d.Label))
	}
	if ok, _ := c.Exists(doctype, newName); ok {
		return "", cerr.Duplicate("{0} {1} já existe", c.T(d.Label), newName)
	}
...
func (c *Ctx) checkEmails(d *meta.DocType, doc Doc) error {
	for _, f := range d.Fields {
		...
		if !validEmail(value) {
			return cerr.Validation("{0}: \"{1}\" não é um endereço de e-mail válido", c.T(f.Label), value).WithTitle(c.T("E-mail inválido"))
		}
	}
	return nil
}
...
func (c *Ctx) checkMandatory(d *meta.DocType, doc Doc) error {
	...
	if len(missing) > 0 {
		return cerr.Mandatory("Preencha os campos obrigatórios: {0}", strings.Join(missing, ", ")).WithTitle(c.T("Campos obrigatórios"))
	}
	return nil
}
...
func (c *Ctx) checkReadOnlyDependsOn(d *meta.DocType, doc, before Doc) error {
	...
	for _, f := range d.Fields {
		...
		return cerr.Validation("{0} é somente leitura neste documento", c.T(f.Label)).WithTitle(c.T("Campo somente leitura"))
	}
	return nil
}
...
func (c *Ctx) checkSelect(d *meta.DocType, doc Doc) error {
	for _, f := range d.Fields {
		...
		if !ok {
			return cerr.Validation("{0}: valor \"{1}\" não está entre as opções", c.T(f.Label), v)
		}
	}
	return nil
}
	...
	for _, f := range d.Fields {
		...
		case "Link":
			if ok, err := c.Exists(f.OptionsString(), v); err != nil {
				...
				return cerr.LinkExists("{0}: {1} \"{2}\" não existe", c.T(f.Label), f.OptionsString(), v).WithTitle(c.T("Link inválido"))
			}
		case "Dynamic Link":
			target := doc.Str(f.OptionsString())
			if target == "" {
				return cerr.Validation("{0}: informe o tipo antes do vínculo", c.T(f.Label))
			}
			if _, err := c.St.DocType(target); err != nil {
				return cerr.Validation("{0}: DocType \"{1}\" não existe", c.T(f.Label), target)
			}
			if ok, err := c.Exists(target, v); err != nil {
				return err
			} else if !ok {
				return cerr.LinkExists("{0}: {1} \"{2}\" não existe", c.T(f.Label), target, v).WithTitle(c.T("Link inválido"))
			}
		}
	}
	return nil
...
func (c *Ctx) checkUnique(d *meta.DocType, doc Doc) error {
	for _, f := range d.DataFields() {
		...
		if len(rows) > 0 {
			return cerr.Duplicate("{0} \"{1}\" já existe em {2} {3}", c.T(f.Label), doc.Str(f.Fieldname), c.T(d.Label), rows[0]["name"]).WithTitle(c.T("Valor duplicado"))
		}
	}
	return nil
}
	...
	for _, tf := range d.TableFields() {
		...
		for i, row := range rows {
			...
			if err := c.checkMandatory(child, row); err != nil {
				return cerr.Validation("{0}, linha {1}: {2}", c.T(tf.Label), i+1, cerr.From(err).Message).WithTitle(c.T("Campos obrigatórios"))
			}
...
func (c *Ctx) checkAllowOnSubmit(d *meta.DocType, before, doc Doc) error {
	for _, f := range d.Fields {
		...
		if f.Fieldtype == "Table" {
			if string(mustJSON(stripChildMeta(before.Children(f.Fieldname)))) != string(mustJSON(stripChildMeta(doc.Children(f.Fieldname)))) {
				return cerr.Validation("Não é permitido alterar {0} depois do envio", c.T(f.Label)).WithTitle(c.T("Documento enviado"))
			}
			continue
		}
		nv, _ := castValue(f, doc[f.Fieldname])
	...
	for _, other := range c.St.Meta.DocTypes {
		for _, f := range other.Fields {
			...
			if len(rows) > 0 {
				...
				return cerr.LinkExists("{0} {1} está vinculado a {2} {3}", c.T(d.Label), name, ref, refName).WithTitle(c.T("Não é possível apagar"))
			}
		}
	}
	return nil
...
func (c *Ctx) writeInsert(d *meta.DocType, doc Doc) error {
	...
	if err != nil && strings.Contains(err.Error(), "duplicate key") {
		return cerr.Duplicate("{0} {1} já existe", c.T(d.Label), doc.Name())
	}
	return err
}
...
func (c *Ctx) writeUpdate(d *meta.DocType, doc Doc, prevModified any) error {
	...
	if err != nil && strings.Contains(err.Error(), "duplicate key") {
		return cerr.Duplicate("Valor duplicado em {0}", c.T(d.Label))
	}
	if err != nil {
		return err
	}
	if tag.RowsAffected() == 0 {
		return cerr.Timestamp("O documento {0} {1} foi alterado por outro usuário. Recarregue e tente de novo.", c.T(d.Label), doc.Name())
	}
	return nil
}
