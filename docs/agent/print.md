...
fields are removed, in the parent and in child rows. Nothing else is. The
standard layout also skips `Hidden` fields, but a custom template receives them,
so a template must not print a hidden field it should not show. Field-level
permissions do not exist yet (SEC-02), so no field is redacted per user.
