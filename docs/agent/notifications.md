...
copy without the fields above that recipient's permission level (see `field-permissions`).
All functions are synchronous: never use `async`, `await` or return a Promise. `condition`
must return a boolean; a non-boolean result throws and rolls back the document operation,
occurrences and jobs together, same as any other evaluation error. A false condition skips
the occurrence.
