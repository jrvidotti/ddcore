...
| Fieldtypes | Data, Email, Small Text, Text, Text Editor (rich text, HTML cleaned by an allowlist), Markdown Editor, Code, Int, Float, Currency, Percent, Rating, Duration, Color, Check, Date, Month, Datetime, Time, Select, Link, Dynamic Link, Table, Table MultiSelect, Attach, Attach Image, JSON, Password, Vault, and layout fields. |
