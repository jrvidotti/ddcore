...
| SEC-04 | Recuperação de senha, convite e proteção do login | Parcial: usuário/senha e redefinição via CLI; não identificado fluxo de recuperação por token nem limitação de tentativas. | P0 para política operacional de acesso e limitação de tentativas; P1 para autosserviço. Definir expiração e revogação de sessões/chaves e testar abuso/usuário desativado. |
...
| SEC-06 | Segredos de integração e campos Password | Parcial: senhas de login/chaves têm hash; campo Password genérico é `text`, sem proteção automática equivalente a um cofre. | P0 se guardar credenciais. Serviço de segredos com criptografia, rotação e referências em configuração; nunca exportar valores para Desk, Version ou logs. |
...
| DAT-04 | Evolução de schema e dados com renomes/conversões | Parcial: DDL automático e patches existem; DDL vem antes dos patches; fixtures inserem ou pulam por nome. | P0: roteiro de conversão que preserve dados. Para transformações incompatíveis, usar expansão → preenchimento → validação → retirada em versões separadas. |
...
| DAT-06 | Precisão decimal e semântica de datas | Parcial para equivalência: `numeric` no Postgres, mas cálculos do app usam números JS; suporte a locale/fuso já existe. | P0 para apps com valores monetários. Definir precisão/arredondamento e comparar somas, parcelas, juros e datas históricas; criar API decimal se os casos exigirem. |
...
| DAT-09 | Custom Fields, Property Setters, Client/Server Scripts | Parcial: TS versionado e scripts de formulário; não identificado editor de customização persistida. | P0 para inventariar e portar customizações existentes; P2 para editor visual. Converter alterações efetivas para arquivos TS; definir extensões entre apps, conflitos e geração de tipos. |
...
| PRD-03 | Observabilidade e saúde operacional | Parcial: logs, Error Log, `doctor`, `/api/health`; este retorna `ok`, sem provar disponibilidade do banco. | P0: readiness do banco, alertas de fila/backup/erros, duração e correlação de requisições; limites e metas de operação definidos para o piloto. |
...
Evidências locais: [CLI](../cmd/ddcore/main.go), [configuração](../internal/config/config.go),
[API](../internal/api/api.go), [jobs](../internal/engine/jobs.go) e [hub](../internal/engine/hub.go).
