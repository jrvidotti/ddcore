...
| Operators: `=`, `!=`, `>`, `<`, `>=`, `<=`, `like`, `not like`, `in`, `not in`, `between`, `is` (`set`/`not set`), `descendants of`, `not descendants of`, `ancestors of`, `not ancestors of`, `timespan`, `fiscal year`, `not between` | `=`, `!=`, `>`, `>=`, `<`, `<=`, `like`, `not like`, `in`, `not in`, `between`, `is`, `set`, `not set`, `descendants of` (`db/query.go:23-27`) | adaptable | S–M | `descendants of` is accepted but compiled as `=`: silently wrong for trees (DAT-07 absent). Missing: `not between`, `timespan`, the ancestor forms. `like` is case- and accent-insensitive (`ILIKE`), unlike MariaDB collation in some setups. |
