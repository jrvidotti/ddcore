...
| SEC-06 | Integration secrets and Password fields | **Implemented**: integration credentials do not live in columns — `ddcore.secret("stripe_key")` reads `DDCORE_SECRET_STRIPE_KEY` from environment (`.env` in dev, platform in prod), and prefix is the boundary preventing apps from reading DSN or SMTP password via same path. `ddcore doctor` lists names and never values. `Password` fields never exposed via reads: `RedactPassword` scrubs value on all API responses, augmenting preexisting exclusion from `Version` and export. Contract in [fieldtypes](agent/fieldtypes.md#secrets-ddcoresecret-not-a-column) and [configuration](../DEVELOPMENT.md). | Residual: integration secrets live outside DB by design — not in backup, replica, export, or diff; rotation is redeployment, not migration. **No** at-rest vault encryption: a `Password` field typed by a *person* remains plaintext in column, protected only on egress. Encrypting at rest or rotating stored credentials requires future work; for Frappe secrets migration, requesting source encryption key separately from data dump remains valid. |
...
| ID | Capability | Status in ddcore | Priority and Minimum Delivery |
|---|---|---|---|
| DAT-01 | Import with mapping, validation, and resumption | No generic importer identified. CRUD/MCP, patches, and fixtures are available primitives. | P0 for pilot migrator. Manifest, dry run, per-record errors, checkpoint, and idempotency; P1 for CSV/XLSX UI. |
| DAT-02 | Complete and reconcilable export | **Implemented**: `Ctx.Export` traverses keyset-filtered dataset with child tables and attachment manifest, exposed via `GET /api/export/<DocType>` (streaming, capped) and `ddcore export` (NDJSON/CSV, attachment bytes, checksums). Contract in [export](agent/export.md). | Delivered. Server-side `export` permission checked; `Password` fields and credential columns never exported. Only corresponding import remains (DAT-01). |
...
| DAT-08 | Additional field types and properties | Partial: v1 set expanded with Month/Password; Text Editor uses textarea. | P2 by usage: rich text, Attach Image, Table MultiSelect, Duration, Geolocation, Barcode, Rating, etc. Map conversion and data fidelity before choosing controls. |
...
| ID | Capability | Status in ddcore | Priority and Minimum Delivery |
|---|---|---|---|
| OPS-01 | Print Format and PDF | No print/template/PDF engine identified. | P1; blocks operations issuing contracts, receipts, or other documents. Default template per DocType, app templates, preview, pagination, locale, and authorization. |
...
| OPS-03 | Notifications by event, date, and recipient | Partial: scheduler, jobs, and SSE; no generic rules or persistent notification inbox. | P1; declarative rules, authorized recipients, read/unread tracking, and deduplication. SSE alone does not deliver notifications to offline users. |
| OPS-04 | Approval workflow | Partial: docstatus and controller methods; no declarative state/transition engine identified. | P1; blocks mandatory approvals. States, actions, roles, conditions, history, and docstatus binding, validated on server. |
...
| OPS-08 | Global search and specialized views | Partial: Link/list search, filters, and navigation; no full global search, Calendar, Kanban, or Gantt equivalents identified. | P2 by workflow; indexing with authorization and saved preferences before multiplying views. |
...
| OPS-10 | Portal, Web Forms, and customer/supplier access | Partial: `allowGuest` methods exist; no portal/web forms product identified. | P2; blocks apps relying on external self-service. Separate profiles and routes, validate abuse and attachments, define explicit publication. |
| OPS-11 | API and library compatibility | Partial: similar REST/RPC model; server uses Go/goja and proprietary bridge. | P0 for active consumers. Map paths, verbs, payloads, errors, auth, and pagination; port Python/Jinja/SQL and replace incompatible Python/Node dependencies. |
