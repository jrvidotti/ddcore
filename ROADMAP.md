...
| OPS-01 | Document print templates and PDF generation: auto-generated standard layout from DocType metadata, app-declared custom print templates in `print/<name>.print.ts` via `definePrintTemplate`, corporate branding via standard Core DocType `Letter Head`, pluggable server-side PDF generation (Gotenberg, custom CLI, local headless Chromium), concurrency limiter semaphore, strict read permissions and field-level redaction (`Password`, `Vault`), Desk preview route `/app/[workspace]/[doctype]/[name]/print` with paper canvas and browser print fallback. See [print templates](docs/agent/print.md). | No visual drag-and-drop template designer; dynamic PDF watermarking and digital signatures remain demand-driven. |
| SEC-01 | User access scopes through the Core `User Permission` DocType: per-user `allow`/`for_value` rules, optionally limited to one DocType, enforced below the SDK on lists, counts, link search, direct reads, insert/update/delete/submit/cancel/amend, export, attachments, versions/comments, SSE events and notifications. Two users with identical roles, System Manager included, stay isolated; `ignorePermissions` skips role checks but not scopes; `Administrator` and framework-internal elevated contexts are unscoped. Grants and revocations are recorded as `permission.scope_grant`/`scope_revoke` audit events. See [scopes](docs/agent/scopes.md). | Direct reads and writes do not check `Dynamic Link` fields, so an out-of-scope document linked only through one can be opened by name; queries do filter them. `ddcore.db.sql` ignores scopes. `is_default` is stored but does not prefill forms, and `allow`/`for_value` are not validated. No test yet covers a report or an enqueued job under a scoped user. Scheduled methods run as `Administrator`. |
...
| OPS-04 | Declarative approval workflows through `defineWorkflow`: states, docstatus binding, role-restricted field edits (`allowEdit`), transitions with actions, allowed roles, `allowSelfApproval`, synchronous TypeScript conditions, server-side transition engine with PostgreSQL row locking, audit logging in timeline comments, and Desk action buttons/state pills. See [workflows](docs/agent/workflows.md). | Multi-level approval chains with visual flowchart editors remain demand-driven. Parallel/forking approval branches are out of scope (workflows are linear state machines). Automatic periodic transition escalation remains demand-driven. |
